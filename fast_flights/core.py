...
logging.basicConfig(level=logging.WARNING)  # Adjust level as needed (DEBUG, INFO, WARNING, ERROR, CRITICAL)
...
def get_flights(
    ...
    **kwargs: Any,
    ...
    # Debugging with logging instead of print
    logging.debug("Calling get_flights")
    ...
    if not results.flights:
        logging.debug(f"No flights found, attempted: {attempted}")  # Log instead of print
        if not attempted:
            return get_flights(
                tfs,
                cookies=cookies,
